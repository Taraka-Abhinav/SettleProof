...
| Stress replay | **Generated result** | measured rows, duration, and throughput in [`benchmark.json`](artifacts/benchmark.json) |
...
Privacy is fail-closed: the normal proof export omits imported raw rows; the optional full export is encrypted in-browser with AES-256-GCM using a passphrase-derived PBKDF2-HMAC-SHA-256 key (310,000 iterations, random salt and IV). The passphrase is never stored or sent. CSP, no-referrer, no-store, frame-denial, MIME-sniffing, cross-origin, and permissions headers reduce browser attack surface. This protects the implemented local workflow; production key custody and durable encrypted storage remain explicit adapter boundaries.
...
The six close-certificate invariants are non-vacuous posting set, record conservation, settlement conservation, bank conservation, ledger conservation, and cash-cutoff conservation. Details: [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md).
...
| [`close_certificate.json`](artifacts/close_certificate.json) | posting-readiness status and all six invariant proofs |
...
- all six close-certificate invariants pass for the safe posting set;
